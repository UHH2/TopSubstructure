...
namespace uhh2examples {
  ...
  private:
    ...
    uhh2::Event::Handle<bool> h_passed_gen, h_passed_rec, h_passed_rec_met_sideband, h_passed_rec_pt_mu_sideband, h_passed_gen_met_sideband, h_passed_gen_pt_mu_sideband;
  };
  ...
  KinCutModule::KinCutModule(Context & ctx){
    ...
    if(isTTbar){
      ...
      gen_met_sideband.reset(new GenMETSelection(ctx, 40,-1));
  ...
  bool KinCutModule::process(Event & event) {
    ...
    if(isTTbar){
      ...
      if(passed_nmu_gen && passed_met_gen){
        h_gen_met->fill(event);
      }
      if(passed_nmu_gen && passed_met_gen && passed_pt_mu_gen){
        h_gen_pt_mu->fill(event);
        passed_gen = true;
        if(genmatching->passes(event)) h_gen_matched->fill(event);
        else h_gen_unmatched->fill(event);
      }
      if(passed_nmu_gen && !passed_met_gen && passed_pt_mu_gen && gen_met_sideband->passes(event)){
        h_gen_met_sideband->fill(event);
        passed_gen_met_sideband = true;
      }
      if(passed_nmu_gen && passed_met_gen && !passed_pt_mu_gen && gen_pt_mu_sideband->passes(event)){
